function dnpp = DNPP(X, x)
global it d 
global dnppCS moiCS rand_cauchy_dnpp

for k = unique([x.I.idx, x.idx]); S{1}(k) = 1; end
if moiCS == 2; S{1}(x.I.idx) = x.I.weight; end % MOI(RANKED)
...
switch dnppCS
    case 0  % rectangular ------------------------------------------------- 
        for k = x.I.idx
            if k == x.idx
                dnpp = dnpp + x.phi(1) * S{1}(k) * Mtx((x.prtInfo - x.pos));
            else
                dnpp = dnpp + X(k,it).phi(2) * S{1}(k) * Mtx((X(k,it).prtInfo - x.pos));
            end
        end 

    case 1  % spherical ---------------------------------------------------
        L = x.pos;
        P = x.pos + x.phi(1) * S{1}(k) * Mtx((x.prtInfo - x.pos)); 
        for k = setdiff(x.I.idx, x.idx)
            L = L + X(k,it).phi(2) * S{1}(k) * Mtx((X(k,it).prtInfo - x.pos));
        end 
        C = (x.pos + L + P)/3;
        R = norm(C - x.pos);
    ...
    case 2  % standard ----------------------------------------------------
        for k = x.I.idx
            q = (x.phi(1) * S{1}(x.idx) * x.prtInfo + x.phi(2) * S{1}(x.idx)...
                * X(k,it).prtInfo ) / (x.phi(1) * S{1}(x.idx) + x.phi(2) * S{1}(x.idx));
            dnpp = dnpp + q - x.pos;
        end
